...
function getSuggestionsUrl(action, params) {
    var suggestUrl = "/help/search/" + action +  "/doccenter";
...
function getDocRoot() {
    if (!window.docroot) {
        window.docroot = getDocRootFromPath();
    }
    return window.docroot;
}

function getDocRootFromPath() {
    var pathname = $(location).attr('href');
    var docroot = /\/help(\/releases\/R20\d\d[ab])?/.exec(pathname);
